...
def insert_ISP(db, cursor, isp_name, user_fullname, user_email, time_now):
...
def get_most_recent_ISP_id(cursor, isp_name, time_now):
    ...
    print("ispID: "+str(ispID))
    return ispID

def insert_DNSs(db, cursor, ispID):
...
def get_most_recent_DNS_ids(cursor):
    dns_ips = listOfDNSs()[1]
    print("dns_ips: "+str(dns_ips))
    list_of_dns_ids_in_database = {}
    for DNS in dns_ips:
        print("DNS: "+str(DNS))
        ...
        list_of_dns_ids_in_database[DNS] = DNS_ID

        print("list_of_dns_ids_in_database: "+str(list_of_dns_ids_in_database))

    return list_of_dns_ids_in_database

def insert_domain(db, cursor, domain, dns_ips):
    ...
    return DNS_ID_List_In_Database



def insert_IPs(db, cursor, domain, DNS_ID_List_In_Database):
...
def convert_domain_to_database(list_of_domain_objects, isp_name, user_fullname, user_email):
    ...
    #Adds 3 instances of the domain to the databse, one for each DNS
